...
model = AutoModelForSequenceClassification.from_pretrained(model_args.model_name_or_path, num_labels=1,
...
def build_dataset(tokenizer, config):
    ...
    ds = load_dataset("json", data_files='/apdcephfs_cq2/share_1603164/data/lingfengshen/data/stack/stack_train_data.json', split="train")
    ds = ds.map(tokenize, batched=False)
    train_dataset = ds.filter(lambda x: len(x["chosen_input_ids"]) <= 512 and len(x["rejected_input_ids"]) <= 512)

    ds = load_dataset("json", data_files='/apdcephfs_cq2/share_1603164/data/lingfengshen/data/stack/stack_eval_data.json', split="train")
    ds = ds.map(tokenize, batched=False)
    eval_dataset = ds.filter(lambda x: len(x["chosen_input_ids"]) <= 512 and len(x["rejected_input_ids"]) <= 512)
    # eval_dataset = None
...
class RMTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        rejected_rewards = \
        model(input_ids=inputs["rejected_input_ids"], attention_mask=inputs["rejected_attention_mask"])[0]
...
## Save model
model_lora.save_pretrained('/apdcephfs_cq2/share_1603164/data/lingfengshen/output/'+'new_rm_llama_7B')