...
eval_dataset = load_dataset('json', data_files="/apdcephfs_cq2/share_1603164/data/lingfengshen/data/stack/stack_train_data.json", split='train')
...
model = AutoModelForSequenceClassification.from_pretrained(
    '/apdcephfs_cq2/share_1603164/data/lingfengshen/trl/llama-7b-reward-new',
...
def preprocess_function(examples):
    ...
    for query, response_j, response_k in zip(examples["question"], examples["response_j"], examples["response_k"]):

        sentence = "Question: " + query + "\n\nAnswer: " + response_j
        length = len(sentence.split())
        print(length)
        aug = nac.RandomCharAug(action="insert",aug_char_min=int(0.15*length), aug_char_max=int(0.15*length))
        #augmented_texts = aug.augment(j_text, n=1)[0]
        typo_sentence = aug.augment(sentence, n=1)[0]

        tokenized_j = tokenizer(sentence, truncation=True)
        tokenized_k = tokenizer(typo_sentence, truncation=True)
