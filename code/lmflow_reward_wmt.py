...
def build_dataset(tokenizer, config):
    ...
    def tokenize(sample):
        tokenized_pos = tokenizer("Please translate the sentences to English: " + sample['question'] + "\n\nAnswer: " + sample['response_j'], truncation=True)
        tokenized_neg = tokenizer("Please translate the sentences to English: " + sample['question'] + "\n\nAnswer: " + sample['response_k'], truncation=True)
    ...
    ds = ds.filter(lambda x: len(x["chosen_input_ids"]) <= 256 and len(x["rejected_input_ids"]) <= 256)
    eval_dataset = None
    if config.validation_split_percentage > 0:
        idx_gap = int((1 - 20 / 100) * len(ds))
...
## Save model
model_lora.save_pretrained('/apdcephfs_cq2/share_1603164/data/lingfengshen/trl/mt_reward')