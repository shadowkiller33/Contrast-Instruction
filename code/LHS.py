...
class LHS(object):
    ...
    def __init__(self, model_name_or_path: str,
                 ...
                 LHS_num: int = 100,
                 LSH_conduct: bool = True,
                 interval: bool = True,
                 threshold: float = 0.02,
                 pooler=None):
        self.model_name_or_path = model_name_or_path
        self.LSH_conduct = LSH_conduct
        ...
        self.vec = torch.randn(LHS_num, self.hidden_size).to(self.device)
        self.point = self.vec / self.vec.norm(dim=1, keepdim=True)

        # = torch.randn(LHS_num, 786)/tensor.norm(dim=1, keepdim=True)
        self.index = None
    ...
    def encode(self, sentence: Union[str, List[str]],
        ...
        with torch.no_grad():
            ...
            for batch_id in tqdm(range(total_batch)):
                ...
                result = self.point.mm(embeddings.t()).t()
                if self.LSH_conduct == True:
                    if self.interval == True:
                        result1 = ((result < self.threshold) & (result > -self.threshold)).long()
                    else:
                        result1 = (result < 0).long()
                    embedding_list.append(result1.cpu())
                else:
                    embedding_list.append(result.cpu())
    ...
    def build_index(self, sentences_or_file_path: Union[str, List[str]],
        ...
        if load:
            ...
            #embedding_path = embedding_path + self.model_name_or_path
            embeddings = self.encode(sentences_or_file_path, device=device, batch_size=batch_size, normalize_to_unit=True, save=True, save_path=embedding_path,
                                 return_numpy=True)
