...
def get_reward(a_j, a_k):
    ...
    rewards_j = model(input_ids=torch.Tensor(inputs["input_ids_j"]).to(torch.float16).cuda(), attention_mask=torch.Tensor(inputs["attention_mask_j"]).to(torch.float16).cuda())[0]
    rewards_k = model(input_ids=torch.Tensor(inputs["input_ids_k"]).to(torch.float16).cuda(), attention_mask=torch.Tensor(inputs["attention_mask_k"]).to(torch.float16).cuda())[0]
